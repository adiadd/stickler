...
from stickler.structured_object_evaluator.model_def.json_schema_validator import (
...
class TestValidateJsonSchema:
    ...
    def test_invalid_schema_bad_structure(self):
        ...
        schema = {"type": "object", "properties": "should be a dict not a string"}
        with pytest.raises(SchemaError):
            validate_json_schema(schema)

    def test_invalid_schema_bad_items(self):
        """Test that invalid items definition raises SchemaError."""
        schema = {"type": "array", "items": "should be a schema not a string"}
        with pytest.raises(SchemaError):
            validate_json_schema(schema)
