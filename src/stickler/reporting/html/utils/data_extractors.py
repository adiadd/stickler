    ...
    def extract_similarity_score(
        ...
        if field_name:
            ...
            return field_data.get(
                "raw_similarity_score", field_data.get("similarity_score", 0.0)
            )
        else:
            # Overall similarity
            if isinstance(results, ProcessEvaluation):
                ...
                return results.get(
                    "similarity_score",
                    results.get("overall", {}).get("similarity_score", 0.0),
                )
    ...
    def extract_field_threshold(model_schema: type, field_name: str) -> Optional[float]:
        ...
        try:
            ...
            # Check JSON schema directly
                ...
                and "x-comparison" in json_schema_extra
            ):
    ...
    def extract_all_field_thresholds(model_schema: type) -> Dict[str, float]:
        ...
        try:
            for field_name in model_schema.__fields__:
                threshold = DataExtractor.extract_field_threshold(
                    model_schema, field_name
                )
                ...
                if field_type and hasattr(field_type, "__fields__"):
                    nested_thresholds = DataExtractor.extract_all_field_thresholds(
                        field_type
                    )
                    for nested_field, nested_threshold in nested_thresholds.items():
                        field_thresholds[f"{field_name}.{nested_field}"] = (
                            nested_threshold
                        )
