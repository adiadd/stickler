...
class SectionGenerator:
        ...
        for metric in key_metrics:
            if metric in metrics:
                ...
                if isinstance(value, float):
                    ...
                    color = (
                        ColorUtils.get_performance_color(value, config.color_thresholds)
                        if hasattr(self.viz_engine, "_get_performance_color")
                        else "#007bff"
                    )
                else:
    ...
    def generate_field_analysis(self, config: ReportConfig) -> str:
        ...
        html_string += self.viz_engine.generate_field_performance_chart(
            field_metrics, config
        )

        # Add color coded performance chart.
        html_string += self.viz_engine.generate_field_performance_table(
            field_metrics, config
        )
    ...
    def generate_non_matches(self, config: ReportConfig) -> str:
        ...
        for nm in displayed:
            ...
            non_match_type = html.escape(
                str(nm.get("non_match_type", "N/A")).replace("NonMatchType.", "")
            )
            ground_truth_value = html.escape(str(nm.get("ground_truth_value", "None")))[
                :100
            ]  # Truncate long values
            prediction_value = html.escape(str(nm.get("prediction_value", "None")))[
                :100
            ]
    ...
    @staticmethod
    def generate_document_gallery(
        document_images: Dict[str, str], config: ReportConfig
    ) -> str:
