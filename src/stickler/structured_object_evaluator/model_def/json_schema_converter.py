...
class JsonSchemaFieldConverter:
    ...
    def convert_property_to_field(
        ...
        if json_type == "object":
            return self._handle_nested_object(
                field_name, property_schema, is_required, field_path
            )

        # Handle arrays
        if json_type == "array":
            return self._handle_array_type(
                field_name, property_schema, is_required, field_path
            )
        ...
        comparator = extensions.get(
            "comparator"
        ) or self._get_default_comparator_for_type(json_type)
    ...
    def _extract_stickler_extensions(
        ...
        if "x-aws-stickler-comparator" in property_schema:
            ...
            try:
                extensions["comparator"] = self._create_comparator_from_name(
                    comparator_name
                )
    ...
    def _handle_nested_object(
        ...
        try:
            NestedModel = StructuredModel._from_json_schema_internal(
                enriched_schema, field_path=field_path
            )
        except ValueError:
        ...
        if items_type == "object":
            ...
            try:
                ElementModel = StructuredModel._from_json_schema_internal(
                    items_schema, field_path=f"{field_path}[]"
                )
            except ValueError:
                # Nested errors already have field path context
                raise
