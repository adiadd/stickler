...
from ..comparison.helpers_base import ComparisonHelperBase
...
class FieldComparisonHelper(ComparisonHelperBase):
    ...
    def _extract_entries_from_objects(
        ...
        if isinstance(gt_object, StructuredModel) and isinstance(
            ...
            for nested_field_name, field_score in comparison_result.get(
                "field_scores", {}
            ).items():
                ...
                expected_key = (
                    f"{field_name}[{gt_index}].{nested_field_name}"
                    if gt_index is not None
                    else f"{field_name}[].{nested_field_name}"
                )
                actual_key = (
                    f"{field_name}[{pred_index}].{nested_field_name}"
                    if pred_index is not None
                    else None
                )

                # Create reason for this specific field
                field_reason = self.generate_comparison_reason(
                    field_is_match, field_score, info.threshold
                )
            ...
            expected_key = (
                f"{field_name}[{gt_index}]"
                if gt_index is not None
                else f"{field_name}[]"
            )
            actual_key = (
                f"{field_name}[{pred_index}]" if pred_index is not None else None
            )

            return [
                self.create_field_comparison_entry(
                    expected_key=expected_key,
